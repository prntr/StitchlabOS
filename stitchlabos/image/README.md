...
| `pico-firmware` | Prebuilt SKR Pico firmware + `stitchlab-flash-pico` |
...
| `stitchlab-intake` | G-code pre-flight checks + thumbnails |
| `stitchlabos` | Final customizations |

## Adding to Raspberry Pi Imager

`os_list.json` is **generated by CI**, not stored here — a checked-in copy cannot
know the release's URL, size or SHA256, and Beta1 shipped one whose placeholders
were never filled in. `os_list.template.json` holds the placeholders; the build
workflow substitutes them and publishes the result as a release asset.

Start Imager against the newest release's list:

```bash
rpi-imager --repo https://github.com/prntr/StitchlabOS/releases/latest/download/os_list.json
```

StitchLabOS then appears under *Choose OS*, with Imager's customisation dialog
available (the image keeps cloud-init, hence `init_format: cloudinit-rpi`).

Alternatively use *Choose OS → Use custom* with the `.img.xz` from the release
page. On that path Imager cannot read an `init_format`, so its customisation
dialog stays greyed out and the image's built-in defaults apply.
